...
Adds an item to the container. The item is placed in the first available slot(s) and can be stacked with existing items of the same type. Note, use [*@minecraft/server.Container.setItem*](../../../priorscriptapi/minecraft/server-1xx/Container.md#setitem) if you wish to set the item in a particular slot.
...
Gets an [*@minecraft/server.ItemStack*](../../../priorscriptapi/minecraft/server-1xx/ItemStack.md) of the item at the specified slot. If the slot is empty, returns `undefined`. This method does not change or clear the contents of the specified slot. To get a reference to a particular slot, see [*@minecraft/server.Container.getSlot*](../../../priorscriptapi/minecraft/server-1xx/Container.md#getslot).
