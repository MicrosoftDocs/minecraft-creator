...
Fogs are a rendering feature that can be used to obscure the player's view distance, to occlude boundaries such as unloaded chunks beyond the render distance, or to produce an atmospheric effect. There are several types of fogs that can be used to produce a desired effect.

Fogs are set up in individual JSON files in a resource pack under the **fogs** directory.
...
Fog settings can be determined for biomes with **biomes_client.json**. In each biome entry, you can have a variable called `fog_identifier` and set a name that matches the fog setting  to use with this biome.
