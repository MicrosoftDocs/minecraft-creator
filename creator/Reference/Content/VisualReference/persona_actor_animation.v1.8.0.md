...
|Name       |Default Value |Type |Description |Example Values |
|:----------|:-------------|:----|:-----------|:------------- |
| anim_time_update | *not set* | String | how does time pass when playing the animation. Defaults to "query.anim_time + query.delta_time" which means advance in seconds. |  | 
| blend_weight | *not set* | String |  |  | 
...
| loop_delay | *not set* | String | How long to wait in seconds before looping this animation. Note that this expression is evaluated after each loop and on looping animation only. |  | 
...
| start_delay | *not set* | String | How long to wait in seconds before playing this animation. Note that this expression is evaluated once before playing, and only re-evaluated if asked to play from the beginning again. A looping animation should use 'loop_delay' if it wants a delay between loops. |  | 
...
| position (Alternate 1) | *not set* | String |  |  | 
| position (Alternate 2) | *not set* | String |  |  | 
...
| rotation (Alternate 2) | *not set* | String |  |  | 
| rotation (Alternate 3) | *not set* | String |  |  | 
| rotation (Alternate 4) | *not set* | Keyed set of strings |  |  | 
| scale | *not set* | Key/item pairs of [Scale](#scale-item-type) items |  |  | 
| scale (Alternate 1) | *not set* | String |  |  | 
| scale (Alternate 2) | *not set* | String |  |  | 
...
| pre_effect_script | *not set* | String | A Molang script that will be run when the particle emitter is initialized |  | 
