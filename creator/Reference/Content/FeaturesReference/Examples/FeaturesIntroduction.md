...
All features must specify the version that they target via the `"format_version"` field. The remainder of the data is contained in independent JSON sub-objects for each supported feature type. These feature types define the behavior of the feature and include properties specific to that behavior. To be valid, a definition must include exactly one of these type objects. (See the [list of supported feature types](#supported-features).)
...
- first\_pass
- before\_underground\_pass
- underground\_pass
- after\_underground\_pass
- before\_surface\_pass
- surface\_pass
- after\_surface\_pass
- before\_sky\_pass
- sky\_pass
- after\_sky\_pass
- final\_pass
