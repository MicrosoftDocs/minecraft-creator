...
|:----------|:----------|:----------|:----------|
|[minecraft:bone_visibility](minecraftBlock_bone_visibility.md)| *not set*| JSON Object| A JSON object that contains a list of key/value pairs mapping from bone names in the specified geometry file (key) to a boolean defining whether the bone should be visible or not (value).|
...
|[minecraft:part_visibility](minecraftBlock_part_visibility.md) | true | Boolean |Maps bone names in a geometry file to a condition that turns their rendering on/off. The condition should be a Molang query that uses block properties to determine true/false. |
...
|[minecraft:rotation](minecraftBlock_rotation.md)| rotation[0, 0, 0]| Vector [a, b, c]| The block's rotation around the center of the cube in degrees. The rotation order is [x, y, z]. Angles need to be in multiples of 90. |
