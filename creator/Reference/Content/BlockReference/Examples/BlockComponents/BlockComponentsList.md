...
|[minecraft:liquid_detection](minecrasftBlock_liquid_detection.md)| *not set* | Identifier string| Defines how a block behaves when detecting liquid. Only one rule definition is allowed per liquid type. If multiple are specified, the first will be used and the rest will be ignored. |
