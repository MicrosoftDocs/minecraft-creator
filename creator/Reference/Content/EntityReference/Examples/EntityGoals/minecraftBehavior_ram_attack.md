...
## Entity Ram Attack Behavior Properties
...
| cooldown_range | {"max":20,"min":10} | [Cooldown Range](#item-components-floatrange) item | Minimum and maximum cooldown time-range (positive, in seconds) between each attempted ram attack. | Goat: `{"min":30,"max":300}`, `{"min":5,"max":15}` | 
...
| on_start | [] | [Start](#entity-actordefinitiontrigger) item | The event to trigger when attacking | Goat: `[{"event":"start_event","target":"self"}]` | 
| pre_ram_sound | undefined | [Pre Ram Sound](#pre-ram-sound-choices) choices | The sound to play when an entity is about to perform a ram attack. | Goat: `"pre_ram"`, `"pre_ram.screamer"` | 
| priority | 0 | Integer number | As priority approaches 0, the priority is increased. The higher the priority, the sooner this behavior will be executed as a goal. | Goat: `5` | 
| ram_distance | 7 | Integer number | The distance at which the mob start to run with ram speed. | Goat: `7` | 
| ram_impact_sound | undefined | [Ram Impact Sound](#pre-ram-sound-choices) choices | The sound to play when an entity is impacting on a ram attack. | Goat: `"ram_impact"`, `"ram_impact.screamer"` | 
...
Minimum and maximum cooldown time-range (positive, in seconds) between each attempted ram attack.  Check that the limits imposed on the range (minimum, maximum and maximum distance between values) are respected.
