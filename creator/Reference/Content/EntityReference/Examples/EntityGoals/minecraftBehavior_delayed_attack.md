...
|:----------|:-------------|:----|:-----------|
| attack_duration | 0.75 | Decimal number | The entity's attack animation will play out over this duration (in seconds). Also controls attack cooldown. | 
...
| hit_delay_pct | 0.5 | Decimal number | The percentage into the attack animation to apply the damage of the attack (1.0 = 100%). Value must be <= 1. | 
...
| reach_multiplier | 1.5 | Decimal number | Used with the base size of the entity to determine minimum target-distance before trying to deal attack damage. | 
...
| sound_event | attack.strong | [Sound Event](#sound-event-choices) choices |  | 
| speed_multiplier | 1 | Decimal number | Multiplier applied to the mob's movement speed when moving toward its target. | 
| track_target | true | Boolean true/false | Allows the mob to track its target even if it lacks a hard-coded sensing component. | 
