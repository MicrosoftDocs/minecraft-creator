...
| loot_table | *not set* | String | The loot table that contains the possible loot the entity can drop with this goal. | Cat: `"loot_tables/entities/cat_gift.json"`, Eliza: `"loot_tables/entities/eliza_gift.json"` | 
...
| on_drop_attempt | *not set* | [Minecraft Event Trigger](../Definitions/NestedTables/triggers.md) | The event to trigger when the entity attempts to drop an item. | Cat: `{"event":"minecraft:cat_gifted_owner","target":"self"}`, Eliza: `{"event":"mike_eliz:eliza_gifted_owner","target":"self"}` | 
