...
> [!Note]
> In format version 1.26.20, `destination_position_range` must be specified as a Float Range object (an object with `range_min` / `range_max`). Legacy scalar or array forms no longer parse.
