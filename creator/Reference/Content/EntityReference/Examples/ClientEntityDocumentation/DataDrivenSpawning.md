...
Data-Driven spawning allows you to adjust the spawn conditions of mobs within Minecraft: Bedrock Edition, giving you the power to determine when, where, and how mobs spawn in your world. Let's take a closer look at how data-driven spawning works.

## Spawn Rules

All Spawn Rules JSON need to have an ID (located under the description section). Similar to other identifiers, these identifiers follow the "namespace:name" convention. The `minecraft` namespace is reserved for the vanilla Minecraft rules. When changing an existing mob, use the ID that appears in the entity JSON for that entity. When creating your own mob, make sure all of the entity's JSON files use the same ID.

Spawn Rules also need to define the pool that is used for population control. Each pool has its own spawn limit. By setting an entity to a pool, it will spawn as long as that pool hasn't reached the spawn limit.
...
> [!NOTE]
> There are 2 more pools located within the game files, `Villager` and `Pillager`. However, these are not currently used by any entities within Minecraft.
...
| [minecraft:delay_filter](../Definitions/NestedTables/delay_filter.md) |Allows players to set mobs spawn with certain time delays before they will spawn.|
...
| [minecraft:difficulty_filter](../Definitions/NestedTables/difficulty_filter.md)| Allows players to set mobs spawn with certain difficulty levels. |
...
| [minecraft:height_filter](../Definitions/NestedTables/height_filter.md) |Allows players to set mobs spawn within certain heights of their map. |
...
| [minecraft:spawns_on_block_filter](../Definitions/NestedTables/spawns_on_block_filter.md) |Allows an entity to spawn on a particular block. Includes a string or array of strings for the block they may spawn on. |
| [minecraft:spawns_on_block_prevented_filter](../Definitions/NestedTables/spawns_on_block_prevented_filter.md) |Prevents and entity from spawning on a particular block. Includes a string or array of strings for the block they may not spawn on. |
| [minecraft:spawns_on_surface](../Definitions/NestedTables/spawns_on_surface.md) |Allows an entity to spawn on the surface. |
| [minecraft:spawns_underwater](../Definitions/NestedTables/spawns_underwater.md)|Allows the mob to spawn underwater. Adding the component sets the value of it to true, while removing it causes the mob to stop spawning underwater.  |
| [minecraft:weight](../Definitions/NestedTables/weight.md)|Allows players to set a priority for how often that mob should spawn. Mobs with lower weight values will have a lower chance to spawn than mobs with higher weight values. |
| [minecraft:world_age_filter](../Definitions/NestedTables/world_age_filter.md) |Allows players to set mobs spawn after a certain amount of time has passed within a world.|
...
Below is a list of all the available spawn rules for entities within the Vanilla Behavior Pack for Minecraft: Bedrock Edition.
