...
To get started, you’ll want to use the Node Package Manager, or npm. NPM makes it easy to download and install different modules of code within your packages. To get started, install Node.js – which includes NPM - on your development device. More info on installing NPM is available from https://nodejs.org. You’ll likely want to install the latest LTS version of Node.js to get started.
...
From here, this will install type definitions in the node_modules folder within your project.

Now, to edit with new code hinting, just open up Visual Studio Code. As you write JavaScript, you should see autocomplete:
...
1. Installing the Minecraft Bedrock Edition Debugger within Visual Studio Code – you’ll need to do this once
...
If you wish to connect Minecraft Bedrock Edition client to Visual Studio Code running on the same machine (this is the most common scenario), you will need to exempt the Minecraft client from UWP loopback restrictions. To do this, run the following from a command prompt or the Start | Run app.
...
While you’re building out gametests - chances are you are deploying them to Minecraft’s development_behavior_packs folder. This is located within `%localappdata%\Packages\Microsoft.MinecraftUWP_8wekyb3d8bbwe\LocalState\games\com.mojang\development_behavior_packs`. Select the behavior pack you’d like to debug, and open up a Visual Studio Code window pointed at that folder.

#### Step 4: Prepare Visual Studio Code for a connection

In order to debug with Minecraft Bedrock Edition, you’ll need to connect from Minecraft and into Visual Studio Code. This sample assumes you are debugging on the same Windows 10 machine, but you can also debug across machines and across clients if you so choose. If you are debugging across devices, you may need to open up a port within your firewall on the machine that you are running Visual Studio Code within.

To configure the connection, add a .vscode subfolder within your behavior pack folder.  Within that .vscode folder, add the following launch.json configuration file:
...
First, hit "Start Debugging" inside of Visual Studio Code. This will put Visual Studio Code into "listen for a debugging connection mode".

Start Minecraft and load into a world with your scripting behavior pack. You may want to set a break point inside of your GameTest function – to do this, clicking on the left-hand side specific lines of code.
...
The procedure for debugging with Bedrock Dedicated Server is a little different – when debugging with Bedrock Dedicated Server, Bedrock Dedicated Server will in this case listen for debug connections initiated from Visual Studio Code. You’ll want to start by installing the Minecraft Bedrock Edition Debugger for Visual Studio Code as described above.
...
Your Visual Studio Code instance should be pointing at the root of your Behavior Pack, within the development_behavior_packs folder of the Minecraft instance (so likely you should open Visual Studio code at `(my Bedrock Dedicated Server installation)\development_behavior_packs\(behaviorpackname)`)

At the root of the behavior pack you wish to debug, add a .vscode subfolder. Add the following launch.json file into that .vscode folder:
(NOTE: you do not need to edit any lines of this file.)
