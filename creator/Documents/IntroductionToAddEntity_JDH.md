...
You can add an entity to Minecraft: Bedrock Edition using a behavior pack and a resource pack. As you learned from the recommended tutorials, the behavior of entities can be changed with a behavior pack and the appearance with a resource pack. Both are required to add a working entity to the game. This guide will go through the basic steps to create a working entity. 

////TODO: update this next sentence later after editing the rest of this document

In the second part, it will show you how to breathe life into the entity by using behavior components, animations, etc.
...
Open the `robot.entity.json` file in the  `entities` folder in your resources pack.

For most entities (such as our robot), we can use the default render controller that's provided by the game. We will talk more about render controllers later in the more advanced half of this tutorial. For now, just know that this is where it goes.
...
Now would be a good time to try the full robot resource and behavior packs. Compare the collection of folders and files. Then, put back your minimal robot packs so we can continue adding functionality. This is also good practice for keeping the different things you're working on organized. It can also help you appreciate practicing good naming habits.
...
If we just want to use one default geometry, material, and texture, we can just leave it to point to the default render controller as we did before. But in this example, I want to show you how to add random textures, so let’s break down how render controllers work first.
