...
In addition, it will be helpful if you have a code editor like [Visual Studio Code](https://code.visualstudio.com/download), and if you're familiar with how the [JSON](https://www.w3schools.com/whatis/whatis_json.asp) format works.
...
The model is referenced by the geometry name. If you create a model in [Blockbench](https://www.blockbench.net/), make sure the geometry name in the project settings is set to your entity name&mdash;in this case, "robot".
...
In this section, we'll add a simple wheel animation to the robot. If you want to learn more about animations, how to use animation controllers, and how to create animations in [BlockBench](https://www.blockbench.net/), read [this guide](EntityModelingAndAnimation.md).
...
Each controller is named using the scheme `controller.render.<entity-name>`. Our one controller is named `controller.render.robot`. For a multi-purpose render controller, we can also use another keyword instead of the entity name.
